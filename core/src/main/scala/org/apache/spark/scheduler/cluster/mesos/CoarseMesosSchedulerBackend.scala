...
private[spark] class CoarseMesosSchedulerBackend(
  ...
  private val maxExecutorsPerSlave =
    conf.getOption("spark.mesos.coarse.executors.max").map { m => m.toInt }.getOrElse(1)
  ...
  private def executorTerminated(
    ...
    stateLock.synchronized {
      if (slaveIdsWithExecutors.contains(slaveId) && taskIdToSlaveId.contains(executorId)) {
        ...
        slaveIdsWithExecutors(slaveId) = slaveIdsWithExecutors(slaveId) - 1
      }
    }
  }
  ...
  override def slaveLost(d: SchedulerDriver, slaveId: SlaveID): Unit = {
    ...
    stateLock.synchronized {
      taskIdToSlaveId.keys.filter(_.startsWith(slaveId.getValue)).foreach { taskId =>
    ...
    for (executorId <- executorIds) {
      if (taskIdToSlaveId.contains(executorId)) {
        mesosDriver.killTask(
          TaskID.newBuilder().setValue(taskIdToSlaveId(executorId).toString).build())
      } else {
